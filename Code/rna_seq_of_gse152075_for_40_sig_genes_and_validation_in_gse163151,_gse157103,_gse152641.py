# -*- coding: utf-8 -*-
"""RNA Seq of GSE152075 for 40 sig genes and validation in GSE163151, GSE157103, GSE152641.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1sCFNBSgPbf_dmngOwhXARQiq_cPcCavJ
"""
...
# For 075 ML model validation in GSE163151, GSE157103, GSE152641
import joblib
from sklearn.metrics import accuracy_score

def validate_models_on_dataset(model_files, X_data, y_data, dataset_name):
    # Load all models for the dataset
    loaded_models = {name: joblib.load(file) for name, file in zip(model_names, model_files)}

    # Get predictions for the dataset
    predictions = {name: model.predict(X_data) for name, model in loaded_models.items()}

    # Calculate accuracies for the dataset
    y_data = y_data.astype(int)
    accuracies = {name: accuracy_score(y_data, preds) for name, preds in predictions.items()}
...
# Define the model names and their associated file paths for GSE152075
model_names = ["XGBoost", "RandomForestClassifier", "LogisticRegression", "SVC"]
model_files = [f"{name}.pkl" for name in model_names]

# Validate the models on each dataset
datasets = {
...
for dataset_name, (X, y) in datasets.items():
    validate_models_on_dataset(model_files, X, y, dataset_name)